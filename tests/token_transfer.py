...
async def phase_three(client, exporter):
    token_transfer_parser = TokenTransferParser(exporter)
    token_transfer_parser.parse(exporter.get_item(EntityType.LOG))
    print(len(exporter.get_item(EntityType.TOKEN_TRANSFER)))
...
async def main():
    ...
    await phase_three(client, exporter)
