...
async def enrich_swaps(swaps, blocks, transactions, service:Service):
    ...
    for result_item in tqdm(result):
        ...
        token0_address = await service.get_token0_address(pool_address)
        token1_address = await service.get_token1_address(pool_address)
        ...
        quote_address, base_address = determine_quote_token(token0_address, token1_address, service)
        ...
        token0_name = await service.get_token_name(token0_address)
        token0_symbol = await service.get_token_symbol(token0_address)
        token0_decimals = await service.get_token_decimals(token0_address)
        token0_total_supply = await service.get_token_total_supply(token0_address)

        token1_name = await service.get_token_name(token1_address)
        token1_symbol = await service.get_token_symbol(token1_address)
        token1_decimals = await service.get_token_decimals(token1_address)
        token1_total_supply = await service.get_token_total_supply(token1_address)
        ...
        if service.checkpoint_liquidity.get(pool_address, None) is not None:
            timestamp = service.checkpoint_liquidity[pool_address]
        ...
        if result_item['call'] != 'pair_created' and service.checkpoint_liquidity.get(pool_address, None) is None:
            ...
            if result_item['dex_exchange'] == 'uniswapv3':
                liquid0 = service.get_token_balance(pool_address, token0_address)
                liquid1 = service.get_token_balance(pool_address, token1_address)
            else:
                liquid0, liquid1 = service.get_liquidity(pool_address)
            ...
            service.checkpoint_liquidity[pool_address] = checkpoint_timestamp
            logger.info(f'pool_address: {pool_address} | checkpoint_liquidity: {service.checkpoint_liquidity[pool_address]}')
