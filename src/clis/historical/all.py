...
async def main(
    ...
    raw_trace_fetcher = RawTraceFetcher(
        client=client, exporter=exporter[EntityType.RAW_TRACE]
    )
    ...
    for batch_start_block in range(start_block, end_block + 1, process_batch_size):
        ...
        if not set(entity_types).isdisjoint([EntityType.RAW_TRACE,EntityType.TRACE]):
            extract_trace(
                exporter,
                raw_trace_fetcher,
                raw_trace_parser,
