...
async def main(start_block, end_block, process_batch_size, request_batch_size, entity_types, exporter_types):
    ...
    for batch_start_block in range(start_block, end_block + 1, process_batch_size):
        ...
        logger.info(f"Num RawTrace: {len(exporter[EntityType.RAW_TRACE])}")
        logger.info(f"Num Trace: {len(exporter[EntityType.TRACE])}")
...
# python -m src.clis.historical.export_traces --start-block 23170000 --end-block 23170030 \
#     --process-batch-size 100 --request-batch-size 5 \
# --entity_types raw_trace,trace --exporter_types duckdb
